...
async function testPoolConnection_Repository(pool) {
    try {
        /* On demande à la base de données de nous renvoyer une clé qui se nomme test et qui vaut 1 */
        const [result] = await pool.query("SELECT 1 as test");
        if (result.length === 1 /* Vérifie que le nombre de lignes retournées est bien 1 */ &&
        ...
        throw new Error("Connexion impossible à la DB, vérifiez vos identifiants de connexion et vos variables d'environnement.");
    }
}
